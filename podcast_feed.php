...
<channel>
    ...
    <title>Sports Time Machine Podcast - <?php echo ucfirst(str_replace('_',' ',$_REQUEST['station']))?></title>
    <description>Sports Time Machine Podcast - <?php echo ucfirst(str_replace('_',' ',$_REQUEST['station']))?> -Stark County Ohio</description>
...
<itunes:author>Sports Time Machine <?php echo str_replace('_',' ',ucfirst($_REQUEST['station']))?></itunes:author>
<itunes:subtitle>Sports Time Machine <?php echo str_replace('_',' ',ucfirst($_REQUEST['station']))?> Podcast</itunes:subtitle>
...
<item>
    ...
    <title><![CDATA[<?php echo str_replace("&nbsp;"," ",stripslashes($podcast['post_title']))?>]]></title>

    <itunes:author>Sports Time Machine <?php echo str_replace('_',' ',ucfirst($_REQUEST['station']))?></itunes:author>

    <itunes:subtitle><?php echo str_replace("&nbsp;"," ",stripslashes(substr(strip_tags($podcast['post_content']),0,150)))?></itunes:subtitle>

    <enclosure url="<?php echo $audio_url?>" type="audio/mpeg" length="<?php echo filesize($audio_abs_path)?>" />
    <itunes:duration><?php echo $duration['minutes']?>:<?php echo $duration['seconds']?></itunes:duration>

    <guid><?php echo wp_get_attachment_url( $audio['audio'][0] );?></guid>

    <pubDate><?php echo date("D, j M Y H:i:s", strtotime($podcast['post_date']))?> EST</pubDate>
