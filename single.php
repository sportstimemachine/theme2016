...
get_header();
?>
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/jplayer/jquery.jplayer.min.js"></script>
<link href="<?php bloginfo('template_url'); ?>/js/jplayer/jplayer.css" rel="stylesheet" type="text/css" />
<link href="<?php bloginfo('template_url'); ?>/js/jplayer/blue_monday/jplayer.blue.monday.css" rel="stylesheet" type="text/css" />

<?php
global $post;
$id =  $post->ID;
$post = get_post_custom($id);
$radio_show = get_post($id);
$radio_show = (array) $radio_show;

$radio = wp_get_attachment_url($post['audio'][0]);
 ?> 						
	                                    ...
	                                    <?php if($radio != ''){
	                                    	...
	                                    	<div style="text-align:center;margin-bottom:10px;">
												<object type="application/x-shockwave-flash" data="/template/player_mp3_mini.swf" width="200" height="20">
												    ...
												    <param name="FlashVars" value="mp3=<?=$radio;?>" />
												</object>
											</div>
	                                    	<?php
	                                    } else{ ?>
	                   <div style="margin-bottom:10px;">
							<div id="jquery_jplayer_<?=$radio_show['ID']?>" class="jp-jplayer"></div>
								<div id="jp_container_<?=$radio_show['ID']?>" class="jp-audio" style="margin: 0px auto">
							...
							<script>
								$().ready(function(){
									$("#jquery_jplayer_<?=$radio_show['ID']?>").jPlayer({
										cssSelectorAncestor: "#jp_container_<?=$radio_show['ID']?>",
							...
							<?php
	                            ...
	                            <div id="disqus_thread"></div>
									<script type="text/javascript">
									    ...
									    var disqus_identifier = '<?=$post->post_type?>-<?=$post->post_name?>';
